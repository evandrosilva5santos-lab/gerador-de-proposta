...
function BonusDialog({ item, onSave, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-xl)', width: 600, maxWidth: '100%', maxHeight: '90vh', overflow: 'auto', padding: 32, fontFamily: 'var(--font-body)' }}>
...
function BonusPage({ db, update }) {
  ...
  return (
    <div>
      ...
      {editing && <BonusDialog item={editing === 'new' ? null : editing} onClose={() => setEditing(null)} onSave={saveB} />}
      {confirm && (
        <DS_B.Dialog title="Excluir bonificação" onClose={() => setConfirm(null)}
          actions={<React.Fragment>
            <DS_B.Button variant="ghost" onClick={() => setConfirm(null)}>Cancelar</DS_B.Button>
            <DS_B.Button variant="primary" onClick={() => { update({ bonus: db.bonus.filter(x => x.id !== confirm.id) }); setConfirm(null); }}>Excluir</DS_B.Button>
          </React.Fragment>}>
          Excluir "{confirm.nome}" da biblioteca?
        </DS_B.Dialog>
      )}
    </div>
  );
